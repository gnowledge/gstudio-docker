...
if [ -d "gstudio" ]; then
    ...
    git branch -r --list | grep $repo_branch  > /dev/null 2>&1   # Mrunal : No redirections here please
    if [ $? -eq 0 ]; then               # If internet connection is available
    ...
    echo -e "\nInfo-msg : Pull the gstudio latest code from github online repo and $repo_branch branch as gstudio directory already exist.\n"  | sed -e "s/^/$(date +%Y%b%d-%I%M%S%p) $   /"  2>&1 | tee -a ${INSTALL_LOG_FILE}
    
    git branch --list | grep $repo_branch  > /dev/null 2>&1   # Mrunal : No redirections here please
    if [ $? -eq 0 ]; then               # If internet connection is available
	git checkout $repo_branch       # Switch to branch 
    else
	git checkout -b $repo_branch    # Create and Switched to branch
    fi	
    #git pull https://github.com/gnowledge/gstudio.git
    git pull origin $repo_branch | sed -e "s/^/$(date +%Y%b%d-%I%M%S%p) $   /"  2>&1 | tee -a ${INSTALL_LOG_FILE}
    cd ..
else    
    # Clone the gstudio code from github online repo
    echo -e "\nInfo-msg : Clone the gstudio code from github online repo : $repo_branch \n"  | sed -e "s/^/$(date +%Y%b%d-%I%M%S%p) $   /"  2>&1 | tee -a ${INSTALL_LOG_FILE}
    git clone https://github.com/gnowledge/gstudio.git -b $repo_branch | sed -e "s/^/$(date +%Y%b%d-%I%M%S%p) $   /"  2>&1 | tee -a ${INSTALL_LOG_FILE}
fi

# Mrunal : for applying comments for sqlite3/postgresql
bash scripts/local_settings_changes.sh

dock_img_name_repo="school-server/$repo_branch";
dock_img_name_tag="v1-$(date +%Y%m%d-%H%M%S)";
dock_img_name="$dock_img_name_repo:$dock_img_name_tag";
dock_img_file=replica-test-20151230-1915.tar;
dock_img=replica-test;
dock_con=school-server-con-$repo_branch-$_TYPE;
...
docker build -t $dock_img_name . | sed -e "s/^/$(date +%Y%b%d-%I%M%S%p) $   /"  2>&1 | tee -a ${INSTALL_LOG_FILE}

docker images $dock_img_name | grep $dock_img_name_tag  > /dev/null 2>&1   # Mrunal : No redirections here please
if [ $? -eq 0 ]; then
...
#    docker run -it -p 8825:25 -p 8822:22 -p 80:80 -p 8443:443 -p 8000:8000 -p 27017:27017 -p 8143:143 -p 8857:587 -p 8025:1025 --name=$dock_con  $dock_img_name
     docker run -it -d  -p $port_ssh_host:$port_ssh_dock -p $port_smtp_host:$port_smtp_dock -p $port_https_host:$port_https_dock -p $port_http_host:$port_http_dock -p $port_django_dev_host:$port_django_dev_dock -p $port_mongo_host:$port_mongo_dock -p $port_smtp_test_host:$port_smtp_test_dock -p $port_imap_host:$port_imap_dock -p $port_smtps_host:$port_smtps_dock --name=$dock_con  $dock_img_name  ;

    $sh_c 'rm docker-inst.lock'   | sed -e "s/^/$(date +%Y%b%d-%I%M%S%p) $   /"  2>&1 | tee -a ${INSTALL_LOG_FILE}       # Image created no need of this file docker-inst.lock
else
